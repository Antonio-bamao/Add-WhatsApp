...
- 进行中：等待部署最新 website/desktop 下载包并让用户重新下载新版 EXE；若微信扫码继续显示 `appid和mch_id不匹配`，需要在 ZPAY/微信支付渠道侧处理 AppID 与商户号绑定，代码侧已经能生成正确 ZPAY 订单链接。
...
- 下一步：生产服务器 `git pull --ff-only` 后构建 website，重启 API 并 reload Nginx；用户完全退出旧托盘进程，重新下载最新 EXE，点击 ZPAY 后应看到支付链接兜底区域。付款测试仍以 ZPAY notify 回调、后台 payment-events 和用户余额增加为准。
