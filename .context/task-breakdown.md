...
2. 补云端工作台 lease 续租/释放 API，并让桌面端在第二工作台存活期间维持租约。
...
6. 继续推进后台真实操作表单、支付/人工入账流程和云端工作台租约续租/释放。
...
- 桌面端已支持云端登录、权益读取、成功添加扣费和第二工作台启动前云端租约申请；当前租约 API 仍是 60 秒一次性 lease，下一步要补续租/释放，避免长期打开的工作台被后台误判为空闲。
