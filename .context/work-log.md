...
## 2026-06-09T01:25:35+08:00｜发布 0.1.7 强制更新弹窗、后台生效时间和桌面图标修复
- 目标：修复强制更新不会主动弹出、后台收费模式生效时间默认排到次日、桌面快捷方式显示 Electron 默认图标的问题，并重新发布安装包和官网更新元数据。
- 后台根因：收费模式面板在当前全站免费时默认把“生效时间”填成次日 00:00，导致管理员切回收费时实际提交的是计划切换，客户端继续免费属于按服务端策略执行。修复为生效时间默认空，并标注“留空立即生效”；只有管理员主动填写未来时间才排程。
- 桌面更新：启动自动检查从 30 秒提前到 5 秒；检测到 mandatory 更新后主窗口会自动 show/focus；renderer 新增不可关闭的强制更新弹窗，下载完成后自动调用安装重启，同时保留“立即重启更新”和异常重试兜底。
- 桌面图标：`assets/icon.ico` 本身正确，但此前未签名构建把 `signAndEditExecutable` 关掉后，exe 资源没有被写入图标，桌面快捷方式可能显示 Electron 默认图标。保留 `signAndEditExecutable=false` 以避开 winCodeSign symlink 权限问题，新增 `scripts/after-pack-icon.js`，在 afterPack 阶段用独立 `rcedit` 把 `assets/icon.ico` 写入 `dist/win-unpacked/Add WhatsApp.exe`。
- 发布：版本升到 `0.1.7`，生成 `dist\Add-WhatsApp-Setup-0.1.7.exe`，大小 `228876916` 字节，SHA256 `dff058a5d8b838fc7096e82e922cd8561d050014f3933128500610313e278c1e`；`npm run publish:update` 已同步 latest alias、`latest/update.json`、`updates/win/stable/Add-WhatsApp-Setup-0.1.7.exe`、blockmap 和 `latest.yml`。
